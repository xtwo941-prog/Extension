...
export function ToastContainer() {
  ...
  return (
    <div style={{ position:'fixed', top:20, right:20, zIndex:9999, display:'flex', flexDirection:'column', gap:8, pointerEvents:'none' }}>
      {toasts.map(t => (
        <div key={t.id} style={{ padding:'10px 16px', borderRadius:8, background:'#18181b', border:`1px solid ${c[t.type]}33`, borderLeft:`3px solid ${c[t.type]}`, color:'#fafafa', fontSize:13, fontWeight:500, boxShadow:'0 8px 24px rgba(0,0,0,0.4)', animation:'toastIn 0.2s ease-out', minWidth:240, maxWidth:360, pointerEvents:'auto' }}>
          {t.message}
        </div>
      ))}
