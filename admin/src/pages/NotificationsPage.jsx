...
export default function NotificationsPage() {
  ...
  async function handleSave() {
    ...
    if (modal.mode === 'create') {
      ...
      ({ error } = await supabase.from('notifications').update(payload).eq('id', modal.data.id));
    }
    setSaving(false);
    ...
    <div>
      ...
      {loading ? (
        ...
        <Table>
          ...
          <Tbody>
            {items.length === 0 && (
              <Tr><Td style={{ textAlign: 'center', color: 'var(--text-3)', padding: 32 }} colSpan={7}>No notifications</Td></Tr>
            )}
            {items.map(n => (
              <Tr key={n.id}>
                <Td style={{ fontWeight: 600, maxWidth: 180 }}>{n.title}</Td>
                <Td style={{ fontSize: 12, color: 'var(--text-2)', maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{n.message}</Td>
                <Td><Badge variant={n.target_status}>{n.target_status}</Badge></Td>
                <Td>
                  <button onClick={() => handleToggle(n)} style={{ padding: '3px 10px', borderRadius: 99, border: 'none', fontSize: 10, fontWeight: 700, cursor: 'pointer', background: n.is_active ? 'rgba(34,197,94,0.15)' : 'rgba(239,68,68,0.1)', color: n.is_active ? '#22c55e' : '#ef4444' }}>
                    {n.is_active ? 'Active' : 'Inactive'}
                  </button>
                </Td>
                <Td style={{ fontSize: 11, color: 'var(--text-3)', maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis' }}>{n.link || '—'}</Td>
                <Td style={{ fontSize: 11, color: 'var(--text-2)' }}>{new Date(n.created_at).toLocaleDateString()}</Td>
                <Td>
                  <div style={{ display: 'flex', gap: 4 }}>
                    <Button size="sm" variant="ghost" onClick={() => openEdit(n)}>Edit</Button>
                    <Button size="sm" variant="danger" onClick={() => handleDelete(n)}>Del</Button>
                  </div>
                </Td>
              </Tr>
            ))}
      ...
      <Modal open={modal.open} onClose={() => setModal({ open: false })} title={modal.mode === 'create' ? 'New Notification' : 'Edit Notification'}>
        ...
        <FormField label="Message">
          <Textarea value={form.message} onChange={e => setForm(p => ({ ...p, message: e.target.value }))} placeholder="Message content..." rows={3} />
        </FormField>
        ...
        <FormField label="Active">
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
            ...
            <span style={{ fontSize: 13, color: 'var(--text-2)' }}>Show to users</span>
          </label>
        </FormField>
