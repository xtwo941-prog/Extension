...
const ALL_STATUSES = ['trial', 'pro', 'lifetime'];
...
export default function FeatureFlagsPage() {
  ...
  function openEdit(f) {
    setForm({
      ...
      allowed_statuses: Array.isArray(f.allowed_statuses) ? f.allowed_statuses : ['pro', 'lifetime'],
    });
  ...
  async function handleSave() {
    if (!form.flag_key.trim()) { showToast('Flag key is required', 'error'); return; }
    setSaving(true);
    const payload = {
      flag_key: form.flag_key.trim().toLowerCase().replace(/\s+/g, '_'),
      description: form.description.trim(),
      is_enabled: form.is_enabled,
      allowed_statuses: form.allowed_statuses,
    };
    let error;
    if (modal.mode === 'create') {
      ({ error } = await supabase.from('feature_flags').insert([payload]));
    } else {
      ({ error } = await supabase.from('feature_flags').update(payload).eq('id', modal.data.id));
    }
    setSaving(false);
    if (error) { showToast(error.message, 'error'); return; }
  ...
  async function handleDelete(f) {
    if (!confirm(`Delete flag "${f.flag_key}"?`)) return;
  ...
  function toggleStatus(s) {
    setForm(p => ({
      ...p,
      allowed_statuses: p.allowed_statuses.includes(s)
        ? p.allowed_statuses.filter(x => x !== s)
        : [...p.allowed_statuses, s],
    }));
  }
    ...
    <div>
      ...
      {loading ? (
        ...
        <Table>
          ...
          <Tbody>
            {flags.length === 0 && (
              <Tr><Td style={{ textAlign: 'center', color: 'var(--text-3)', padding: 32 }} colSpan={6}>No flags</Td></Tr>
            )}
            {flags.map(f => (
              <Tr key={f.id}>
                <Td>
                  <code style={{ fontSize: 12, background: 'var(--bg-surface)', padding: '2px 6px', borderRadius: 4, border: '1px solid var(--border)', color: 'var(--accent)' }}>
                    {f.flag_key}
                  </code>
                </Td>
                <Td style={{ color: 'var(--text-2)', fontSize: 12 }}>{f.description || '—'}</Td>
                <Td>
                  <button
                    onClick={() => handleToggle(f)}
                    style={{
                      width: 44, height: 24, borderRadius: 99, border: 'none', cursor: 'pointer',
                      background: f.is_enabled ? '#22c55e' : 'var(--bg-hover)',
                  ...
                  >
                    <span style={{
                      ...
                      left: f.is_enabled ? 23 : 3,
                    }} />
                  </button>
                </Td>
                <Td>
                  <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                    {Array.isArray(f.allowed_statuses) && f.allowed_statuses.map(s => (
                      <Badge key={s} variant={s}>{s}</Badge>
                    ))}
                  </div>
                </Td>
                <Td style={{ fontSize: 11, color: 'var(--text-3)' }}>{new Date(f.updated_at).toLocaleDateString()}</Td>
                <Td>
                  <div style={{ display: 'flex', gap: 4 }}>
                    <Button size="sm" variant="ghost" onClick={() => openEdit(f)}>Edit</Button>
                    <Button size="sm" variant="danger" onClick={() => handleDelete(f)}>Del</Button>
                  </div>
                </Td>
              </Tr>
            ))}
      ...
      <Modal open={modal.open} onClose={() => setModal({ open: false })} title={modal.mode === 'create' ? 'New Feature Flag' : 'Edit Feature Flag'}>
        <FormField label="Flag Key" hint="lowercase_with_underscores">
          <Input
            value={form.flag_key}
            onChange={e => setForm(p => ({ ...p, flag_key: e.target.value }))}
            placeholder="download_files, shield_mode..."
            disabled={modal.mode === 'edit'}
          />
        </FormField>
        <FormField label="Description">
          <Input value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} placeholder="Short description of this feature..." />
        </FormField>
        <FormField label="Allowed Tiers" hint="Select which license tiers can access this feature">
        ...
        <FormField label="Enabled">
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
            ...
            <span style={{ fontSize: 13, color: 'var(--text-2)' }}>Feature is enabled globally</span>
          </label>
        </FormField>
