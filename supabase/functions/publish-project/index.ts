...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: flag } = await supabase
      ...
      .eq("flag_key", "publish_project")
      .maybeSingle();

    if (flag && flag.is_enabled === false) {
      return jsonRes({ success: false, error_display: "Publishing is currently disabled." }, 403);
    }

    if (flag && Array.isArray(flag.allowed_statuses) && !flag.allowed_statuses.includes(lic.status)) {
      return jsonRes({ success: false, error_display: "Your plan does not include project publishing." }, 403);
    }
    ...
    const res = await fetch(`https://api.lovable.dev/projects/${project_id}/publish`, {
      method: "POST",
      headers,
      body: JSON.stringify({}),
    });
    ...
    return jsonRes({ success: true, data, message: "Project published successfully." });
  } catch (err) {
