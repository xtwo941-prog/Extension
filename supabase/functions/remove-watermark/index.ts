...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { license_key, token_lovable, project_id, session_headers } = body;
    ...
    const { data: flag } = await supabase
      ...
      .eq("flag_key", "remove_watermark")
      .maybeSingle();

    if (flag && flag.is_enabled === false) {
      return jsonRes({ success: false, error_display: "This feature is currently disabled." }, 403);
    }

    if (flag && Array.isArray(flag.allowed_statuses) && !flag.allowed_statuses.includes(lic.status)) {
      return jsonRes({ success: false, error_display: "Your plan does not include watermark removal." }, 403);
    }
    ...
    const res = await fetch(`https://api.lovable.dev/projects/${project_id}/remove-watermark`, {
      method: "POST",
      headers,
      body: JSON.stringify({}),
    });
    ...
    return jsonRes({ success: true, data, message: "Watermark removed successfully." });
  } catch (err) {
